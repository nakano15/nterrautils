...
namespace nterrautils
{
    public class QuestInterface : LegacyGameInterfaceLayer
    {
        ...
        static void ChangeSelectedQuest(int NewQuest)
        {
            ...
            List<QuestData> quests = MainMod.GetPlayerCharacter().GetModPlayer<PlayerMod>().QuestDatas;
        ...
        {
            InterfaceHelper.DrawSeparator(Position, Length, Horizontal, DarkBlueColor ? Color.Blue : Color.Cyan);
        }
        
        private static void DrawBackgroundPanel(Vector2 Position, int Width, int Height, Color color)
        {
            InterfaceHelper.DrawBackgroundPanel(Position, Width, Height, color);
        }
    }
}